...
import { footer } from "./applyResource/citizenFooter";
...
import { sellmeatapplicantSummary } from "./summaryResource/sellmeatapplicantSummary";
import { waterTankerApplicationSummary } from "./summaryResource/watertankerapplicationSummary";
...
const setSearchResponse = async (
  ...
  dispatch(
    prepareFinalObject("Booking", get(response, "bookingsModelList", [])),
  );
  dispatch(
  ...
  console.log(nocStatus, "nocStatus");
  
  localStorageSet("app_booking_status", nocStatus);
...
const setSearchResponseForNocCretificate = async (
  ...
  if (resApproved.length != 0) nocStatus = "APPROVED";
  ...
  if (nocStatus == "APPROVED") {
    ...
    if (get(response0SELLMEAT, "ResposneInfo.status", "") == "") {
      ...
      const response1SELLMEAT = await getSearchResultsForNocCretificate([
        ...
        },
        {
          key: "requestUrl",
          value:
            "/pdf-service/v1/_create?key=sellmeat-noc&tenantId=" + tenantId,
        },
      ]);
      ...
      const response2SELLMEAT = await getSearchResultsForNocCretificateDownload(
        [
          ...
          },
          {
            key: "requestUrl",
            value:
              "/filestore/v1/files/url?tenantId=" + tenantId + "&fileStoreIds=",
          },
        ]
      );
    ...
    let certificateDownloadObjectSELLMEAT = {
      ...
      link: () => {
        if (httpLinkSELLMEAT != "") window.location.href = httpLinkSELLMEAT;
      },
      leftIcon: "book",
    };
  ...
  components: {
    div: {
      ...
      children: {
        ...
        //         undertakingButton1,
		//       }),
		body: role_name === "CITIZEN" ?
			 nocStatus !== "PENDINGPAYMENT"
		  ? getCommonCard({
			  sellmeatapplicantSummary: waterTankerApplicationSummary,
			  taskStatusSummary: taskStatusSummary,
			})
		  : getCommonCard({
			  estimateSummary: estimateSummary,
    ...
    adhocDialog: {
      ...
      props: {
        open: false,
        maxWidth: "sm",
        screenKey: "waterbooking-search-preview",
      },
      children: {
        popup: {},
        //popup:adhocPopup1
