...
const callbackforsummaryaction = async (state, dispatch) => {
  ...
  if (action == 'submit') {
    ...
    const reviewUrl = `${appendUrl}/egov-services/advertisementnoc-my-applications`;
...
const callbackforsummaryactionpay = async (state, dispatch) => {
  ...
  const appendUrl =
    process.env.REACT_APP_SELF_RUNNING === "true" ? "/egov-ui-framework" : "";
  const reviewUrl = parseInt(exempted) === 1 ? `/egov-services/advertisementnoc-my-applications`
    :
    localStorageGet('app_noc_status') === 'REASSIGN' ? `/egov-services/advertisementnoc-my-applications` 
    : `${appendUrl}/egov-services/pay?applicationNumber=${applicationid}&tenantId=${tenantId}`;
  dispatch(setRoute(reviewUrl));
