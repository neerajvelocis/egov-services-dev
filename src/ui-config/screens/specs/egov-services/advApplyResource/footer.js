...
const setReviewPageRoute = (state, dispatch, applnid) => {
  ...
  if (applicationNumber) {
    ...
    const reviewUrl = `${appendUrl}/egov-services/advertisement_summary?applicationNumber=${applicationNumber}&tenantId=${tenantId}`;
    ...
    const appendUrl =
      process.env.REACT_APP_SELF_RUNNING === "true" ? "/egov-ui-framework" : "";
    const reviewUrl = `${appendUrl}/egov-services/advertisement_summary?applicationNumber=${applnid}&tenantId=${tenantId}`;
    dispatch(setRoute(reviewUrl));
  }
};
