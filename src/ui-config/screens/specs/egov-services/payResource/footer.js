...
export const callPGService = async (state, dispatch, item) => {
    ...
    const oldBill = get(
        state,
        "screenConfiguration.preparedFinalObject.oldBill",
        []
    );
    ...
    if (isAvailable) {
        ...
        try {
            ...
            try {
                const userMobileNumber = get(
                    state,
                    "auth.userInfo.mobileNumber"
                );
