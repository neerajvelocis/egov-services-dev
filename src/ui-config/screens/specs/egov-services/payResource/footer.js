...
export const callPGService = async (state, dispatch, item) => {
    ...
    try {
        ...
        for (let index = 0; index < Accountdetails.length; index++) {
            const element = Accountdetails[index];
            if (
                element.taxHeadCode === `OSBM` ||
                element.taxHeadCode === `WTB`
            ) {
                localStorageSet("amount", element.amount);
            } else if (
                element.taxHeadCode === `PETNOC_TAX` ||
        ...
        try {
            ...
            // set(payload, "businessService", businessService);
            // set(payload, "bkAction", "PAY");
            // set(payload, "bkAmount", "350");
            // set(payload, "bkCgst", "63");
            ...
            // Update status for booking Ends
