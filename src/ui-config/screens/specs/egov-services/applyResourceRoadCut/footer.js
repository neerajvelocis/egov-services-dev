...
const setReviewPageRoute = (state, dispatch, applnid) => {
  ...
  if (applicationNumber) {
    ...
    const reviewUrl = `${appendUrl}/egov-services/roadcutnoc_summary?applicationNumber=${applicationNumber}&tenantId=${tenantId}`;
    dispatch(setRoute(reviewUrl));
  }
  else {
    const appendUrl =
      process.env.REACT_APP_SELF_RUNNING === "true" ? "/egov-ui-framework" : "";
    const reviewUrl = `${appendUrl}/egov-services/roadcutnoc_summary?applicationNumber=${applnid}&tenantId=${tenantId}`;
    dispatch(setRoute(reviewUrl));
  }
};
