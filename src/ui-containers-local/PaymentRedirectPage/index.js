...
class PaymentRedirect extends Component {
  componentDidMount = async () => {
    ...
    try {
	  ...
	  {
        this.props.setRoute(
          `/egov-services/acknowledgement-sellmeat?purpose=${"pay"}&status=${"success"}&applicationNumber=${consumerCode}&tenantId=${tenantId}&secondNumber=${transactionId}`
        );
	  }
		else if(getapplicationType() === 'ROADCUTNOC')
	  {
        this.props.setRoute(
          `/egov-services/acknowledgement-roadcut?purpose=${"pay"}&status=${"success"}&applicationNumber=${consumerCode}&tenantId=${tenantId}&secondNumber=${transactionId}`
        );
	  }
	  else
	  {
